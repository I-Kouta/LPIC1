...
- **マネジメントポート**  
ルータやネットワークスイッチが備える通信ポート(端子)の1つで、コンピュタと直接繋いで管理者が操作を行うためのもの。管理用コンピュータとLANケーブルで直に繋いで通信する。岐器内部の管理システムにアクセスでき、CLIで対話的に設定変更などの操作を行うことができる。GUIから管理メニューにアクセスできる機種もある
